...
### Paramètres IA individuels et migration

Le menu **Mes paramètres IA** (`/ai/settings`) permet à chaque utilisateur de gérer ses clés Mistral/Gemini, modèles, RPS, RPM, RPD, TPM et tailles de lots. Dans **Administration → Paramètres IA par utilisateur**, l’administrateur peut ouvrir et modifier ces mêmes réglages pour chaque utilisateur, y compris lui-même. Une clé vide conserve l’existante ; la case Effacer la supprime. Les clés enregistrées ne sont jamais renvoyées dans les champs HTML.

Les paramètres sont stockés dans `users[].ai_settings`. Une configuration utilise toujours les paramètres de son `owner`, même si un administrateur la lance. Une analyse manuelle utilise l’utilisateur connecté. Aucun repli sur une clé globale ou celle de l’administrateur n’est permis. Les réservations de quotas persistées et les délais après 429 sont isolés par propriétaire ; les tâches d’un même propriétaire partagent ses limites.
